...
import {ShieldUfo} from "../objects/ShieldUfo";
...
export class GameScene extends Phaser.Scene {
	...
	private static NextShieldUfo: number = 9;
	...
	private _shieldUfos: Phaser.Physics.Arcade.Group;
	...
	private reset(isFirstLoad: boolean = false) {
		...
		if (this._shieldUfos) {
			let list: Missile[] = this._shieldUfos.getChildren() as Missile[];
			for (let i=0; i < list.length; i++) {
				list[i].setVisible(false);
			}
		}
	...
	public preload(): void {
		...
		/**
		 * Load new shield ufo sprite sheet that I made for the shield animation. Each frame is 32 by 32.
		 */
		this.load.spritesheet('shieldUfo', 'assets/ShieldUfoDataFile.png', { frameWidth: 32, frameHeight: 32 });
	...
	public create(): void {
		...
		/**
		 * Creates new shield ufo objects the same way as with normal ufos
		 */
		this._shieldUfos = this.physics.add.group({ collideWorldBounds: false });
		this._shieldUfos.createMultiple({
			frameQuantity: 5, key: "shieldUfo", frame: 0, visible: false, active: false, classType: ShieldUfo
		});
		...
		/**
		 * Create new animations for disabling and re-enabling shield ufos shields.
		 */
		this.anims.create({
			key: "shield_disabled",
			frames: this.anims.generateFrameNumbers("shieldUfo", { start: 1, end: 39 }),
			frameRate: 39
		});
		this.anims.create({
			key: "shield_enabled",
			frames: this.anims.generateFrameNumbers("shieldUfo", { start: 39, end: 0 }),
			frameRate: 19.5
		});
		this.anims.create({
			key: "explode",
		...
		setTimeout(() => {
			this.physics.add.collider(
				this._missiles, this._ufos, (m: Missile, u: UFO) => { this.onMissileHit(m, u, null); }
			)
			/**
			 * Add new collider for when missiles hit a shield ufo. Calls the onMissileHit function with ufo as null and
			 * shield ufo as the shield ufo that was hit.
			 */
			this.physics.add.collider(
				this._missiles, this._shieldUfos, (m: Missile, u: ShieldUfo) => { this.onMissileHit(m, null, u); }
			)
		}, 100);
	}
	...
	public update(time: number, delta: number): void {
		...
		if (!GameScene.GameRunning) {
			...
			if (!this._uiPanelShowing) {
				...
				let shielded: ShieldUfo = this._shieldUfos.getFirstAlive();
				while (shielded) {
					shielded.setVelocityY(0);
					shielded.setActive(false);
					shielded = this._shieldUfos.getFirstAlive();
				}
		...
		/**
		 * Added a global variable NextShieldUfo which keeps track of how many normal ufos have been send out since the
		 * last shield ufo. This way shield ufo's are only sent out every 10 normal ufos.
		 */
		this._arrivalTimer -= delta;
		if (this._arrivalTimer < 0) {
			if (GameScene.NextShieldUfo != 0) {
				let ufo: UFO = this._ufos.getFirstDead(false);
				if (ufo) {
					this.sendUfo(ufo);
					GameScene.NextShieldUfo -= 1;
				}
			} else {
				let shieldUfo: ShieldUfo = this._shieldUfos.getFirstDead(false);
				if (shieldUfo) {
					this.sendUfo(shieldUfo);
					GameScene.NextShieldUfo = 9;
				}
			}
		}
	...
	/**
	 * Separated this section of code into a new function to save a lot of duplicated code. No way to have a variable
	 * outside of an if else that could be set to either ufo or shield ufo as they are different classes so call this
	 * method instead. Works when ufo is a shield ufo and normal ufo as the functions are shared by both classes.
	 * @param ufo
	 */
	private sendUfo(ufo) {
		...
		let speed = GameScene.UFO_Speed[GameScene.Difficulty] *0.5;
	...
	/**
	 * Modified this function so that it can take both a normal ufo and a shield ufo now that there is two types.
	 * Depending on the type of ufo sent to the function the other will just be null. Added null checks to ensure that
	 * the right ufo is being used.
	 * @param missile
	 * @param ufo
	 * @param shieldUfo
	 */
	private async onMissileHit(missile: Missile, ufo: UFO, shieldUfo: ShieldUfo) {
		let xPos: number;
		let yPos: number;
		let ufoKilled: boolean = false;
		...
		if(shieldUfo && shieldUfo.getShieldActive()) {
			let speed = GameScene.UFO_Speed[GameScene.Difficulty] *0.5;
			shieldUfo.destroyShield(speed)
		} else if (shieldUfo && !shieldUfo.getShieldActive()) {
			shieldUfo.kill();
			ufoKilled = true;
		} else {
			ufo.kill();
		...
		let explosion;
		if (ufo) {
			explosion = this.add.sprite(ufo.x, ufo.y, 'explosion');
		} else {
			explosion = this.add.sprite(shieldUfo.x, shieldUfo.y, 'explosion');
